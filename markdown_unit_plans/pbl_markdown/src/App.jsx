...
export default function App() {
  ...
  const handleRunChain = async () => {
    ...
    log("Starting chain...");
    try {
      let p = window.prompts_pbl;
      if (language === 'sr-Latn') p = window.prompts_pbl_sr_Latn;
      else if (language === 'sr-Cyrl') p = window.prompts_pbl_sr_Cyrl;
      else if (language === 'es') p = window.prompts_pbl_es;
      else if (language === 'ru') p = window.prompts_pbl_ru;
      else if (language === 'id') p = window.prompts_pbl_id;
      if (!p) p = window.prompts_pbl;
      ...
      if (window.utils && window.apiClient && p) {
        ...
        // Fetch PBL Markdown
        log("Calling .NET API to generate PBL Markdown...");
        let currentMarkdown = "";
        try {
          const resStep0 = await fetch("http://localhost:5000/api/pbl/generate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              UnitTitle: name,
              PblJson: JSON.stringify(step0Obj, null, 2),
              Language: language
            })
          });
          if (resStep0.ok) {
            ...
            log("[OK] PBL Markdown generated and displayed.");
          } else {
            log("[WARN] Failed to generate PBL Markdown.");
          }
        } catch (err) {
          log("[WARN] Error fetching PBL Markdown: " + err.message);
        }
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ marginBottom: '8px' }}>PBL Playground</h1>
      ...
      <div style={{ background: '#fff', border: '1px solid #d2d2d7', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          ...
          <div>
            ...
            <TextInput type="number" value={numberOfDays} onUpdate={setNumberOfDays} />
          </div>
          <div>
            <label>Location</label>
            <TextInput value={location} onUpdate={setLocation} />
          </div>
        </div>
