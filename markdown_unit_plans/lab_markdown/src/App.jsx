...
export default function App() {
  ...
  const handleRunChain = async () => {
    ...
    log("Starting chain...");
    try {
      let p = window.prompts_lab;
      if (language === 'sr-Latn') p = window.prompts_lab_sr_Latn;
      else if (language === 'sr-Cyrl') p = window.prompts_lab_sr_Cyrl;
      else if (language === 'es') p = window.prompts_lab_es;
      else if (language === 'ru') p = window.prompts_lab_ru;
      else if (language === 'id') p = window.prompts_lab_id;
      if (!p) p = window.prompts_lab;
      ...
      if (window.utils && window.apiClient && p) {
        ...
        let currentMarkdown = "";
        try {
          const resStep0 = await fetch("http://localhost:5000/api/lab/generate/step0", {
            method: "POST",
        ...
        const res = await fetch("http://localhost:5000/api/lab/generate/lessons", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ marginBottom: '8px' }}>Lab Playground</h1>
