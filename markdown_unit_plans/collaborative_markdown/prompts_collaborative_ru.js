...
- Instruction:
  ...
  - Структура должна естественно перетекать с подсказками Say/Do/Ask/Listen for/Write.
  - ВАЖНО: Не включайте заголовки В ВЕРХНЕМ РЕГИСТРЕ (например, HOOK, INTRODUCTION и т. п.) для разделов.
  - ВАЖНО: Не включайте временные длительности для отдельных инструкций или шагов.
  - TranscendentThinking: Дайте один вопрос о реальном применении, связывающий обучение с целью/смыслом, затем добавьте метку 'Expected Student Responses:' и 2–3 примера.
- GroupStructureAndRoles:
  - Вывод ДОЛЖЕН быть ориентирован на учителя.
  - GroupSize: укажите 'pairs', 'triads' или '4-5 students'.
  ...
  - Rotation: Одно предложение, указывающее, когда роли меняются в ЭТОМ уроке (например, "Rotate roles after Phase A and again before the gallery walk.").

CollaborativeActivities:
- Создайте взаимозависимую совместную деятельность (совместная замена Guided Practice), соответствующую объёму этого урока.
- Каждый учащийся должен внести вклад, и группы должны создать общий продукт или принять общее решение.
- Включите временные ориентиры, сценарий teacher Say:, подсказки для обхода групп + ожидаемые ответы, а также быструю проверку, где ОТВЕЧАЮТ ВСЕ учащиеся + ожидаемые ответы.
        ...
        "CollaborativeActivities": {
          ...
          "properties": {
            ...
            "InstructionsForTeachers": {
              "x-format": "**📋 {loc.InstructionsForTeachers}**\n\n{items}",
              "type": "array",
              "description": "Сценарий учителя для совместной деятельности (приблизительно 6-8 пронумерованных шагов). Убедитесь, что один шаг явно называется 'Circulation Prompts:' и включает конкретные вопросы, которые следует задавать группам.",
              "items": {
                ...
                "properties": {
                  ...
                  "CirculationPrompts": {
                    "x-format": "\n{items}",
                    "type": "array",
                    "description": "ЗАПОЛНЯЙТЕ ЭТО ТОЛЬКО ЕСЛИ Instruction — это 'Circulation Prompts:'. Перечислите конкретные вопросы, которые следует задавать группам во время обхода. НЕ ВКЛЮЧАЙТЕ это свойство, если оно неприменимо.",
                    "items": {
