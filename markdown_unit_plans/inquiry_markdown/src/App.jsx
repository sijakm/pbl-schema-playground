...
export default function App() {
  ...
  const [name, setName] = useState(window.APP_DEFAULTS?.en?.name || "Engineering & Innovation in Ancient Egypt");
  ...
  const handleRunChain = async () => {
    ...
    log("Starting chain...");
    try {
      let p = window.prompts_inquiry;
      if (language === 'sr-Latn') p = window.prompts_inquiry_sr_Latn;
      else if (language === 'sr-Cyrl') p = window.prompts_inquiry_sr_Cyrl;
      else if (language === 'es') p = window.prompts_inquiry_es;
      else if (language === 'ru') p = window.prompts_inquiry_ru;
      else if (language === 'id') p = window.prompts_inquiry_id;
      if (!p) p = window.prompts_inquiry;
      ...
      if (window.utils && window.apiClient && p) {
        ...
        try {
          ...
          const resStep0 = await fetch("http://localhost:5000/api/inquiry/generate/step0", {
            method: "POST",
        ...
        const lessonJsonPromises = lessonsList.map((L, i) =>
          limit(() =>
            withRetry(async (signal) => {
              ...
              if (lessonObj && lessonObj.LessonDescription) {
                lessonObj.LessonDescription.LessonNumber = L.lessonNumber ?? (i + 1);
                lessonObj.LessonDescription.LessonTitle = L.lessonTitle ?? "";
              }
        ...
        console.log(`=== .NET API REQUEST: Lessons ===`);
          ...
          const res = await fetch("http://localhost:5000/api/inquiry/generate/lessons", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
            LessonJsons: lessonJsons.map(obj => JSON.stringify(obj, null, 2)),
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ marginBottom: '8px' }}>Inquiry Playground</h1>
