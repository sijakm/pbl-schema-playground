...
CRITICAL RULES:
1. DO NOT translate any parameters enclosed in double curly braces (e.g., {{$Subject}}, {{$ParentUnitData}}). Keep them exactly as they are.
...
async function processAll() {
  for (const dir of directories) {
    ...
    for (const file of files) {
      ...
      if (!obj.PER_LESSON_PROMPT_TEMPLATE) {
        console.log(`  No PER_LESSON_PROMPT_TEMPLATE found, skipping.`);
        continue;
      }
      
      // We translate the PER_LESSON_PROMPT_TEMPLATE
      const textToTranslate = obj.PER_LESSON_PROMPT_TEMPLATE;
      ...
      try {
        ...
        ['STEP0_PROMPT_TEMPLATE', 'PER_LESSON_PROMPT_TEMPLATE'].forEach(tmplKey => {
          if (obj[tmplKey]) {
            let stringVal = tmplKey === 'PER_LESSON_PROMPT_TEMPLATE' ? translatedText : obj[tmplKey];
        ...
        console.log(`  Done writing translated PER_LESSON to ${file}`);
