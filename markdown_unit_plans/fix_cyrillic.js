...
async function transliterateArray(stringsArr) {
  if (stringsArr.length === 0) return [];
  
  const prompt = `You are a helpful assistant. The following JSON array contains strings that are mostly in Serbian Latin. Your task is to convert the Serbian Latin text into Serbian Cyrillic.
CRITICAL RULES:
1. ONLY transliterate the Serbian words into Cyrillic (e.g. "Samo nalepite" -> "Само налепите").
2. DO NOT translate or transliterate any parameters enclosed in double curly braces (e.g., {{$Subject}}). Keep them exactly as they are.
3. DO NOT translate or transliterate placeholders like {loc.Something} or {value.Something}.
4. DO NOT transliterate English acronyms or terms (e.g., DOK, K-W-L, 'Say:'). Leave them in Latin script.
5. You must return a JSON object with a "translations" array containing the exact same number of strings in the exact same order.`;
      ...
      json_schema: {
        ...
        schema: {
          ...
          properties: {
            translations: {
              type: "array",
              description: "The transliterated strings in the EXACT SAME ORDER as provided. MUST be exactly the same length as the input array.",
              items: { type: "string" }
            }
          },
...
async function processAll() {
  for (const dir of directories) {
    ...
    const files = fs.readdirSync(dirPath).filter(f => f.endsWith('_sr_Cyrl.js'));
    
    for (const file of files) {
      const fullPath = path.join(dirPath, file);
      console.log(`\nProcessing ${file} for Cyrillic transliteration...`);
      ...
      try {
        console.log(`  Calling OpenAI for transliterations...`);
        ...
        for (let i = 0; i < allDescriptions.length; i += chunkSize) {
          ...
          while (!success && attempts < 3) {
            attempts++;
            console.log(`    Translating chunk ${Math.floor(i / chunkSize) + 1} (${chunk.length} items, Attempt ${attempts})...`);
            
            try {
              const translatedChunk = await transliterateArray(chunk);
          ...
          if (!success) {
            throw new Error(`Failed to translate chunk ${Math.floor(i / chunkSize) + 1} after 3 attempts.`);
          }
        }
        ...
        console.log(`  Done saving transliterated schemas to ${file}`);
...
processAll().then(() => console.log("\\nAll Serbian Cyrillic schema descriptions transliterated successfully!"));
