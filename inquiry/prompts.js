...
Grade Level: {{$GradeLevel}}
Duration of class period in minutes: {{$ClassDuration}}
Requested Number of Lessons (context only): {{$NumberOfLessons}}
Standards to Align (use verbatim if present; do NOT add new standards): {{$Standards}}
Students with individualized support (MUST be used ONLY inside Investigation Phase.AccommodationsAndModifications; use the student names/plans exactly as written): {{$LearningPlans}}
Resources/Media to use: {{$MediaContext}}
Unit Content: {{$AttachedUnit}}

Unit-Level Elements from Step 0 (use verbatim):
...
Current Lesson Container from Step 0 (use verbatim as constraints):
- Lesson Number (0-based): {{$LessonNumber}}
- Lesson Title: {{$LessonTitle}}
- Lesson Outline / Scope & Boundaries: {{$LessonOutline}}
...
<p><strong>📚Materials:</strong></p>
<ul>
  - Render each item from Investigation Phase.Materials as a <li>. If empty, output <li>None</li>.
</ul>
...
<ul>
  - Convert Investigation Phase.AnticipatedMisconceptions into multiple <li> items (split into separate misconceptions if needed). Do NOT include numbering.
</ul>

<p><strong>🪜Differentiation</strong></p>
<ul>
  - Convert Investigation Phase.Differentiation into exactly 3 <li> items: (1) Language Learners, (2) Additional Scaffolding, (3) Go Deeper. If the input is already structured, preserve meaning and clean wording.
</ul>
...
<ul>
  <li>{Render Investigation Phase.AccommodationsAndModifications.General}</li>
</ul>

<p><strong>Individual Supports:</strong></p>
<ul>
  - For each item in Investigation Phase.AccommodationsAndModifications.IndividualSupport render:
  <li><strong>{StudentName}:</strong> {Plan}</li>
</ul>

<p><strong>✔Quick Checks</strong></p>
<ul>
  <li>{Render Investigation Phase.QuickCheck}</li>
</ul>
