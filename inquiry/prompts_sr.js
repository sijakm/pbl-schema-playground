...
- Niz Lessons MORA sadržati tačno {{$NumberOfLessons}} lekcija.
...
Standardi za usklađivanje (koristi doslovno ako su prisutni; NEMOJ dodavati nove standarde): {{$Standards}}
Učenici sa individualizovanom podrškom (MORA se koristiti SAMO unutar Investigation Phase.AccommodationsAndModifications; koristi imena učenika/planove tačno onako kako su napisani): {{$LearningPlans}}
Resursi/Mediji za korišćenje: {{$MediaContext}}
Sadržaj unit-a: {{$AttachedUnit}}

Elementi na nivou unit-a iz Koraka 0 (koristi doslovno):
- UnitDescription.Description: {{$UnitDescriptionDescription}}
...
- UnitDescription.StudentLearningObjectives: {{$UnitStudentLearningObjectives}}
- UnitDescription.KeyVocabulary: {{$UnitKeyVocabulary}}
- UnitDescription.StandardsAligned: {{$UnitStandardsAligned}}

Trenutni kontejner lekcije iz Koraka 0 (koristi doslovno kao ograničenja):
- Broj lekcije (indeks 0): {{$LessonNumber}}
- Naslov lekcije: {{$LessonTitle}}
- Okvir lekcije / Obim i granice: {{$LessonOutline}}
...
- AssessPriorKnowledge: SAMO ako je {{$LessonNumber}} == 1, napiši 150–250 reči i prati zahtevanu strukturu u opisu šeme; u suprotnom vrati "".
...
<p><strong>📚 Materijali:</strong></p>
<ul>
  - Renderuj svaku stavku iz Investigation Phase.Materials kao <li>. Ako je prazno, izbaci <li>Nema</li>.
</ul>
...
<ul>
  - Pretvori Investigation Phase.AnticipatedMisconceptions u više <li> stavki (podeli u zasebne zablude ako je potrebno). NEMOJ uključivati numerisanje.
</ul>

<p><strong>🪜 Diferencijacija</strong></p>
<ul>
  - Pretvori Investigation Phase.Differentiation u tačno 3 <li> stavke: (1) Učenici kojima je jezik strani, (2) Dodatna podrška (skela), (3) Napredniji nivo. Ako je ulaz već strukturiran, sačuvaj značenje i očisti formulaciju.
</ul>
...
<ul>
  <li>{Renderuj Investigation Phase.AccommodationsAndModifications.General}</li>
</ul>

<p><strong>Individualna podrška:</strong></p>
<ul>
  - Za svaku stavku u Investigation Phase.AccommodationsAndModifications.IndividualSupport renderuj:
  <li><strong>{StudentName}:</strong> {Plan}</li>
</ul>

<p><strong>✔ Brze provere</strong></p>
<ul>
  <li>{Renderuj Investigation Phase.QuickCheck}</li>
</ul>
