(() => {
  ...
  const SchemaEditor = {
    ...
    init() {
      ...
      const p = lang === "en" ? window.promptsEN : window.promptsSR;
      if (p) {
  ...
  async function runChain() {
    ...
    try {
      ...
      const unitHtmlPrompt = fillTemplate(basePrompts.UNIT_COMMON_HTML_PROMPT_TEMPLATE, {
      ...
      const lessonHtmlPromises = lessonJsons.map((lessonObj, i) =>
        limit(() =>
          withRetry(async (signal) => {
            const ti0 = nowMs();
            const lessonHtmlPrompt = fillTemplate(basePrompts.HTML_LESSON_PROMPT_TEMPLATE, {
