(() => {
  ...
  async function runChain() {
    ...
    try {
      ...
      const unitHtmlPrompt = window.utils.fillTemplate(basePrompts.UNIT_COMMON_HTML_PROMPT_TEMPLATE, {
      ...
      const lessonHtmlPromises = lessonJsons.map((lessonObj, i) =>
        limit(() =>
          withRetry(async (signal) => {
            const ti0 = nowMs();
            const lessonHtmlPrompt = window.utils.fillTemplate(basePrompts.HTML_LESSON_PROMPT_TEMPLATE, {
