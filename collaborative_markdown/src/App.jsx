  ...
  const handleRunChain = async () => {
    ...
    try {
      if (window.utils && window.apiClient && window.promptsEN) {
        ...
        let currentMarkdown = "";
        try {
          const resStep0 = await fetch("http://localhost:5200/api/generate/step0", {
            method: "POST",
        ...
        const res = await fetch("http://localhost:5200/api/generate/lessons", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (res.ok) {
          ...
          setMarkdown("# Failed to fetch from .NET API\nCheck your .NET server running on port 5200.");
        }
      } else {
